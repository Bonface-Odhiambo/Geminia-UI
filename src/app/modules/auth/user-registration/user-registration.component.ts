...
// In a real application, this would be in its own file: auth.service.ts
@Injectable({
  providedIn: 'root',
})
export class AuthService {
  private readonly VALID_USERS = [
    { username: 'individual@geminia.com', password: 'password123', type: 'individual' },
    { username: 'intermediary@geminia.com', password: 'password456', type: 'intermediary' },
  ];

  // This function would be called by a login component.
  login(username: string, password: string): boolean {
    const user = this.VALID_USERS.find(u => u.username === username && u.password === password);
    if (user) {
...
@Component({
    ...
    standalone: true,
    imports: [ CommonModule, ReactiveFormsModule, RouterLink, CurrencyPipe, DecimalPipe, MatDialogModule, PaymentModalComponent ],
...
export class MarineCargoQuotationComponent implements OnInit {
    ...
    // --- Constants ---
    private readonly STAMP_DUTY_FIXED = 40;
    ...
    private setupFormSubscriptions(): void { this.quotationForm.get('modeOfShipment')?.valueChanges.subscribe((mode) => { this.quotationForm.get('destination')?.setValue(mode === 'sea' ? 'Mombasa, Kenya' : mode === 'air' ? 'JKIA, Nairobi, Kenya' : ''); }); this.quotationForm.get('tradeType')?.valueChanges.subscribe((type) => { if (type === 'export') this.showExportModal = true; }); this.quotationForm.get('origin')?.valueChanges.subscribe((country) => { if (this.blacklistedCountries.includes(country)) { this.highRiskRequestForm.patchValue({ originCountry: country }); this.showHighRiskModal = true; } }); this.quotationForm.get('ucrNumber')?.valueChanges.subscribe((ucr) => { this.importerDetails = this.quotationForm.get('ucrNumber')?.valid ? { name: 'Global Imports Ltd.', kraPin: 'P051234567X' } : { name: '', kraPin: '' }; }); }
    private calculatePremium(): void { const sumInsured = this.quotationForm.get('sumInsured')?.value || 0; const productValue = this.quotationForm.get('marineProduct')?.value; const selectedProduct = this.marineProducts.find((p) => p.name === productValue); const rate = selectedProduct ? selectedProduct.rate : 0; const basePremium = sumInsured * rate; const { PHCF_RATE, TRAINING_LEVY, COMMISSION_RATE } = this.TAX_RATES; const phcf = basePremium * PHCF_RATE; const trainingLevy = basePremium * TRAINING_LEVY; const stampDuty = this.STAMP_DUTY_FIXED; const commission = this.currentUser.type === 'intermediary' ? basePremium * COMMISSION_RATE : 0; const totalPayable = basePremium + phcf + trainingLevy + stampDuty; this.premiumCalculation = { basePremium, phcf, trainingLevy, stampDuty, commission, totalPayable, currency: 'KES' }; }
    ...
    downloadQuote(): void { if (this.clientDetailsForm.valid) { this.showToast('Quote download initiated successfully.', 'success'); }}
    ...
    private openPaymentModal(): void {
        const dialogRef = this.dialog.open(PaymentModalComponent, {
            data: { amount: this.premiumCalculation.totalPayable, phoneNumber: this.clientDetailsForm.get('phoneNumber')?.value, reference: `GEM${Date.now()}`, description: 'Marine Cargo Insurance' },
            panelClass: 'payment-dialog-container', autoFocus: false
        });

        dialogRef.afterClosed().subscribe((result: PaymentResult | null) => {
            if (result?.success) {
                this.showToast('Payment successful! Your certificate is being prepared.', 'info');
    ...
    downloadCertificate(): void {
        this.showToast('Your policy certificate has been downloaded.', 'info');
