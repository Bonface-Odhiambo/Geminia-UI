...
@Component({
    ...
    template: `<div class="payment-modal-container">
        <div class="modal-header">
            ...
            <div>
                ...
                <p class="modal-subtitle">
                    Pay KES {{ data.amount | number: '1.2-2' }} for
                    {{ data.description }}
                </p>
            </div>
            <button
                ...
                ><mat-tab
                    ><ng-template mat-tab-label
                        ><div class="tab-label-content">
                            <mat-icon>phone_iphone</mat-icon><span>M-PESA</span>
                        </div></ng-template
                    >
                    <div class="tab-panel-content">
                        <div class="sub-options">
                            <button
                                (click)="mpesaSubMethod = 'stk'"
                        ...
                        <div
                            *ngIf="mpesaSubMethod === 'stk'"
                            class="option-view animate-fade-in"
                        >
                            <p class="instruction-text">
                                Enter your M-PESA phone number to receive a
                                payment prompt.
                            </p>
                            ...
                            <button
                                mat-raised-button
                                class="action-button"
                                (click)="processStkPush()"
                                [disabled]="stkForm.invalid || isProcessingStk"
                            >
                                <mat-spinner
                                    *ngIf="isProcessingStk"
                                    diameter="24"
                                ></mat-spinner
                                ><span *ngIf="!isProcessingStk"
                                    >Pay KES
                                    {{ data.amount | number: '1.0-0' }}</span
                                >
                            </button>
                        </div>
                        <div
                            *ngIf="mpesaSubMethod === 'paybill'"
                            class="option-view animate-fade-in"
                        >
                            <p class="instruction-text">
                                Use the details below on your M-PESA App to
                                complete payment.
                            </p>
                            <div class="paybill-details">
                                <div class="detail-item">
                                    <span class="label">Paybill Number:</span
                                    ><span class="value">853338</span>
                                </div>
                                <div class="detail-item">
                            ...
                            <button
                                mat-raised-button
                                class="action-button"
                                (click)="verifyPaybillPayment()"
                                [disabled]="isVerifyingPaybill"
                            >
                                <mat-spinner
                                    *ngIf="isVerifyingPaybill"
                                    diameter="24"
                                ></mat-spinner
                                ><span *ngIf="!isVerifyingPaybill"
                                    >Verify Payment</span
                                >
                            </button>
                        </div>
                ...
                ><mat-tab
                    ...
                    <div class="tab-panel-content animate-fade-in">
                        <div class="card-redirect-info">
                            <p class="instruction-text">
                                You will be redirected to pay via
                                <strong>I&M Bank</strong>, our reliable and
                                trusted payment partner.
                            </p>
                            <button
                                mat-raised-button
                                class="action-button"
                                (click)="redirectToCardGateway()"
                                [disabled]="isRedirectingToCard"
                            >
                                <mat-spinner
                                    *ngIf="isRedirectingToCard"
                                    diameter="24"
                                ></mat-spinner
                                ><span *ngIf="!isRedirectingToCard"
                                    >Pay Using Credit/Debit Card</span
                                >
                            </button>
                        </div>
                    </div></mat-tab
    ...
    styles: [
        `
            ...
            .payment-tabs .tab-label-content {
                display: flex;
                align-items: center;
                gap: 8px;
                height: 60px;
            ...
            .action-button {
                ...
                height: 52px;
                border-radius: 12px;
                background-color: var(--pantone-2758c) !important;
                color: var(--white-color) !important;
                font-size: 16px;
                font-weight: 700;
            }
            .action-button:disabled {
                background-color: #a0a3c2 !important;
            ...
            ::ng-deep .payment-tabs .mat-mdc-tab-header {
                --mat-tab-header-inactive-ripple-color: rgba(4, 178, 225, 0.1);
                --mat-tab-header-active-ripple-color: rgba(4, 178, 225, 0.2);
            }
            ::ng-deep .payment-tabs .mdc-tab__text-label {
                color: var(--dark-gray);
                font-weight: 600;
            }
            ::ng-deep
            ...
            ::ng-deep .payment-tabs .mat-mdc-tab-indicator-bar {
...
export class MarineCargoQuotationComponent implements OnInit {
    ...
    readonly marineProducts: MarineProduct[] = [
        ...
        {
            code: 'ICC_C',
            name: 'Institute Cargo Clauses (C) - Limited Perils',
            rate: 0.0025,
        },
    ];
    ...
    // CORRECTED: Removed PHCF and updated STAMP_DUTY to be a rate instead of fixed amount
    ...
    private createQuotationForm(): FormGroup {
        return this.fb.group({
            ...
            ucrNumber: [
                '',
                [Validators.required, Validators.pattern('^UCR\\d{7,}$')],
            ],
            idfNumber: [
                '',
                [Validators.required, Validators.pattern('^E\\d{9,}$')],
            ],
        });
    }
    private createClientDetailsForm(): FormGroup {
        return this.fb.group({
            ...
            phoneNumber: [
                '',
                [Validators.required, Validators.pattern('^07[0-9]{8}$')],
            ],
    ...
    private calculatePremium(): void {
        ...
        const selectedProduct = this.marineProducts.find(
            (p) => p.name === productValue
        );
        ...
        // Calculate fees based on sum insured
        const phcf = sumInsured * 0.05;
        const stampDuty = sumInsured * STAMP_DUTY; // Now 5% of sum insured
        const trainingLevy = basePremium * TRAINING_LEVY;
        const commission =
            this.currentUser.type === 'intermediary'
                ? basePremium * COMMISSION_RATE
                : 0;
    ...
    onExportRequestSubmit(): void {
        if (this.exportRequestForm.valid) {
            this.closeAllModals();
            this.showToast(
                'Export request submitted. Our underwriter will contact you.',
                'info'
            );
        }
    }
    onHighRiskRequestSubmit(): void {
        if (this.highRiskRequestForm.valid) {
            this.closeAllModals();
            this.showToast(
                'High-risk shipment request submitted for review.',
                'info'
            );
        }
    }
    closeAllModals(): void {
        ...
        this.quotationForm
            .get('tradeType')
            ?.setValue('import', { emitEvent: false });
        this.quotationForm.get('origin')?.setValue('', { emitEvent: false });
        this.exportRequestForm.reset({
    ...
    downloadCertificate(): void {
        this.showToast(
            'Your policy certificate has been downloaded.',
            'success'
        );
