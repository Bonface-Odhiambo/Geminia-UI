...
export class MarineCargoQuotationComponent implements OnInit {
  ...
  private productCategoryRates: { [key: string]: number } = {
    electronics: 0.0025, machinery: 0.0030, pharmaceuticals: 0.0035,
    chemicals: 0.0040, automotive: 0.0022, textiles: 0.0020,
    general_merchandise: 0.0020, canned_foods: 0.0018, food: 0.0018,
    raw_materials: 0.0015, furniture: 0.0025,
  };

  public productCategories: { [key: string]: string } = {
    electronics: 'Electronics', machinery: 'Machinery & Equipment',
    pharmaceuticals: 'Pharmaceuticals', chemicals: 'Chemical Products',
    automotive: 'Automotive Parts', textiles: 'Textiles & Clothing',
    general_merchandise: 'General Merchandise', food: 'Food & Beverages',
    raw_materials: 'Raw Materials', furniture: 'Furniture & Home Goods',
    canned_foods: 'Canned Foods',
  };

  premiumCalculation: PremiumCalculation = {
    basePremium: 0, addOnPremium: 0, netPremium: 0,
    phcf: 0, trainingLevy: 0, stampDuty: 0, totalPayable: 0,
    baseRate: 0, currency: 'Kshs',
  };
  ...
  private createQuotationForm(): FormGroup {
    return this.fb.group({
      ...
      concealedLossCover: [false], storageWarehouse: [false], warRisk: [false],
      generalAverage: [false], territorialExtension: [false],
  ...
  private noPastDatesValidator(control: AbstractControl): { [key: string]: boolean } | null {
    if (!control.value) {
      return null;
    }
    // Compare date strings in 'YYYY-MM-DD' format to avoid timezone issues.
    if (control.value < new MarineCargoQuotationComponent(null, null).getToday()) {
  ...
  private setupFormSubscriptions(): void {
    this.quotationForm.valueChanges.subscribe(() => {
      if (this.quotationForm.get('sumInsured')?.value > 0 && this.quotationForm.get('productCategory')?.value && !this.isExportMode) {
    ...
    this.quotationForm.get('tradeType')?.valueChanges.subscribe(value => {
      ...
      Object.keys(this.quotationForm.controls).forEach(key => {
        if (key !== 'tradeType') {
          this.isExportMode ? this.quotationForm.get(key)?.disable() : this.quotationForm.get(key)?.enable();
        }
      });
    });
  ...
  private calculatePremium(): void {
    ...
    let baseRate = this.productCategoryRates[formValues.productCategory] || 0.0025;
    if (formValues.coverType === 'ICC_B') baseRate *= 0.85;
    if (formValues.coverType === 'ICC_C') baseRate *= 0.70;
    ...
    this.premiumCalculation = {
      basePremium, addOnPremium, netPremium, phcf,
      trainingLevy, stampDuty, totalPayable, baseRate,
      currency: 'Kshs',
    };
  }
  ...
  private resetPremiumCalculation(): void {
    this.premiumCalculation = {
      basePremium: 0, addOnPremium: 0, netPremium: 0, phcf: 0,
      trainingLevy: 0, stampDuty: 0, totalPayable: 0,
      baseRate: 0, currency: 'Kshs',
    };
  }
  ...
  public getCoverTypeDisplay(value: string): string {
    const coverTypes: { [key: string]: string } = {
      ICC_A: 'ICC(A) - All Risks', ICC_B: 'ICC(B)', ICC_C: 'ICC(C)',
    };
